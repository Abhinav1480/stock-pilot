...
export default function PurchaseOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["purchase-orders", page, pageSize, search, statusFilter],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/orders/purchase?${params}`);
      return res.json();
    },
  });
      ...
      render: (o) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => router.push(`/orders/purchase/${o.id}`)}><Eye className="mr-2 h-4 w-4" /> View Details</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`${data?.meta?.total ?? 0} total purchase orders`}
        actions={
          <Button asChild><Link href="/orders/purchase/new"><Plus className="mr-2 h-4 w-4" />New Order</Link></Button>
        }
      />
      <DataTable
        columns={columns}
        ...
        rowKey={(item) => item.id}
        onRowClick={(item) => router.push(`/orders/purchase/${item.id}`)}
        emptyMessage="No purchase orders found"
        emptyAction={
          <Button variant="outline" size="sm" className="mt-2" asChild>
            <Link href="/orders/purchase/new">Create first order</Link>
          </Button>
        }
        filters={
