...
export function PurchaseOrderDetailsClient({ order: initialOrder }: PurchaseOrderDetailsProps) {
  const router = useRouter();
  const [order, setOrder] = useState(initialOrder);

  const mutation = useMutation({
    mutationFn: async (status: string) => {
      const res = await fetch(`/api/orders/purchase/${order.id}`, {
        method: "PATCH",
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              {order.expectedDate && (
                <div>
                  <span className="text-xs text-muted-foreground block">Expected Date</span>
